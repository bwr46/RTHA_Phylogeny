...
## average coverage for toepads is 3.77... may need to play around with min meanDP levels a bit

for mpileupVCF in *.raw.mpileup.vcf; do
    filteredVCF=${mpileupVCF%.raw.mpileup.vcf}.filtered.vcf
    echo $filteredVCF
    vcftools --vcf $mpileupVCF --max-missing 0.8 --maf 0.05 --min-meanDP 4 --max-meanDP 50 --minQ 35 --min-alleles 2 --max-alleles 2 --remove-indels --recode --out $filteredVCF 2>&1 | tee ${mpileupVCF%.raw.mpileup.vcf}.filter.$(date +%Y%m%d-%Hh%Mm%Ss).log &
    sleep 15
done
